...
wheel = [0]*8 + [list(map(int, input())) for _ in range(4)]
...
for i in range(K):
    ...
    if num == 1:
        if dir == 1:    # 1번을 시계방향으로 돌렸을 때
            tmp = wheel[1][7]
            for j in range(0,7):
                wheel[1][j+1] = wheel[1][j]
            wheel[1][0] = tmp

            if wheel[1][2] == wheel[2][6]:  #1번 2번 맞닿은 곳이 같은 극일 때
                continue
            else:                           #1번 2번 맞닿은 곳이 다른 극일 때
                tmp = wheel[2][0]
                for j in range(6,-1,-1):
                    wheel[2][j] = wheel[2][j+1]
                wheel[2][7] = tmp
                ...
                if wheel[2][2] == wheel[3][6]:
                    ...
                    if wheel[3][2] == wheel[4][6]:
                        ...
                        tmp = wheel[4][0]
                        for j in range(6, -1, -1):
                            wheel[4][j] = wheel[4][j + 1]
                        wheel[4][7] = tmp

        else:           # 1번을 반시계방향으로 돌렸을 때
            tmp = wheel[1][0]
            for j in range(6, -1, -1):
                wheel[1][j] = wheel[1][j + 1]
            wheel[1][7] = tmp
