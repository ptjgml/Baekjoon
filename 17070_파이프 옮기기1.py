...
dp = [[[0]*3 for _ in range(N)] for _ in range(N)]  # dp[r][c][d] : (r, c) 위치에 d방향(0:가로, 1:세로, 2:대각선)으로 도달하는 경우의 수
...
for r in range(N):
    for c in range(N):
        ...
        # 대각선: ↖ ↘ (2)
        if r - 1 >= 0 and c - 1 >= 0:
            if state[r][c] == 0 and state[r][c-1] == 0 and state[r-1][c] == 0:
