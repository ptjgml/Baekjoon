...
#  a는 1 이상 100 이하이다. 뒤따라 나오는 a개의 정수는 어린이 A가 낸 딱지의 그림을 나타내는데, 각각 4, 3, 2, 1 중 하나의 값이다. 
#  4, 3, 2, 1의 순서대로 주어지지 않을 수 있음에 주의하라.

#  다음 줄에는 라운드 1에서 어린이 B가 내는 딱지에 나온 그림의 총 개수 b가 주어진다. 
#  b도 1 이상 100 이하이다. 뒤따라 나오는 b개의 정수는 어린이 B가 낸 딱지의 그림을 나타내는데, 역시 4, 3, 2, 1 중 하나의 값이다. 
#  역시 4, 3, 2, 1의 순서대로 주어지지 않을 수 있음에 주의하라. 다음 두 줄에는 라운드 2에서 어린이 A, B가 낸 딱지의 그림들을 같은 식으로 표현한다. 
...
# 출력하는 글자는 A, B, D 중 하나로 라운드 i의 결과를 나타낸다. 각 라운드의 결과는 A가 승자라면 A, B가 승자라면 B, 무승부라면 D이다.
A = {4:0, 3:0, 2:0, 1:0}
B = {4:0, 3:0, 2:0, 1:0}
...
for i in range(N):
    ...
    for j in range(1,5):
        A[j] = A_input[1:].count(j)
        B[j] = B_input[1:].count(j)

    for k in range(4, 0, -1):
        if A[k] != B[k]:        # 도형의 개수가 다르면 A와 B 비교
            if A[k] > B[k]:
                print('A')
            else:
                print('B')
            ...
            if k == 1:             # k==1 이면 둘이 무승부이므로 D 출력
                print('D')
                break
            else:                   # k가 1이 아니면 다음 도형 비교해야 하므로 위로 올라감
                continue
