...
<<<<<<< HEAD
# 주사위의 모양은 모두 크기가 같은 정육면체이며
#  각 면에는 1부터 6까지의 숫자가 하나씩 적혀있다. 
# 그러나 보통 주사위처럼 마주 보는 면에 적혀진 숫자의 합이
# 반드시 7이 되는 것은 아니다.

# 주사위 쌓기 놀이는 아래에서부터 
# 1번 주사위, 2번 주사위, 3번 주사위, … 의 순서로 쌓는 것이다.
...
# 주사위를 쌓고자 한다. 
# 이렇게 하기 위하여 각 주사위를 위 아래를 고정한 채 
# 옆으로 90도, 180도, 또는 270도 돌릴 수 있다. 
=======
...
>>>>>>> a08c967c050d0f806719ae21c6ea6d464d4033b2
# 한 옆면의 숫자의 합의 최댓값을 구하는 프로그램을 작성하시오.

# 입력
# 첫줄에는 주사위의 개수가 입력된다. 
<<<<<<< HEAD
# 그 다음 줄부터는 한 줄에 하나씩 주사위의 종류가 1번 주사위부터 
# 주사위 번호 순서대로 입력된다. 
# 주사위의 종류는 각 면에 적혀진 숫자가 
# 그림1에 있는 주사위의 전개도에서 A, B, C, D, E, F 의 순서로 입력된다.
...
# 주사위의 종류는 각 면에 적혀진 숫자가 그림1에 있는 주사위의 전개도에서 
# A, B, C, D, E, F 의 순서로 입력된다. 
# 입력되는 숫자 사이에는 빈 칸이 하나씩 있다. 
# 주사위의 개수는 10,000개 이하이며 종류가 같은 주사위도 있을 수 있다.
