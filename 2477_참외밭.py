...
# 반시계방향으로 
# 남쪽으로 30m, 동쪽으로 60m, 남쪽으로 20m, 동쪽으로 100m, 북쪽으로 50m, 서쪽으로 160m 이동하면 
# 다시 출발점으로 되돌아가게 된다.

# 위 그림의 참외밭 면적은 6800m^2이다. 
# 만약 1m^2의 넓이에 자라는 참외의 개수가 7이라면, 이 밭에서 자라는 참외의 개수는 47600으로 계산된다.
...
# 지나는 변의 방향과 길이 (1 이상 500 이하의 정수) 가 
