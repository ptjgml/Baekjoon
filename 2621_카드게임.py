...
#1
if len(set(card_color)) == 1 and card_num[0] == card_num[1]-1 == card_num[2]-2 == card_num[3]-3 == card_num[4]-4:
    result = max(result, card_num[-1] + 900)

#2, 3
elif len(set(card_num)) == 2:
    #3
    if card_num[1] != card_num[3]:
        if card_num[1] == card_num[2]:
            result = max(result, card_num[2]*10 + card_num[3] + 700)
        else:
            result = max(result, card_num[2]*10 + card_num[1] + 700)
    
    #2
    else:
        result = max(result, card_num[2] + 800)

#4
elif len(set(card_color)) == 1:
    result = max(result, card_num[-1] + 600)

#5
elif card_num[0] == card_num[1]-1 == card_num[2]-2 == card_num[3]-3 == card_num[4]-4:
    result = max(result, card_num[-1] + 500)

#6, 7
elif len(set(card_num)) == 3:
    #6
    if card_num.count(card_num[2]) == 3:
        result = max(result, card_num[2] + 400)

    #7
    else:
        result = max(result, card_num[3] * 10 + card_num[1] + 300)

#8
elif len(set(card_num)) == 4:
    for i in range(5):
        if card_num.count(card_num[i]) == 2:
            result = max(result, card_num[i] + 200)
            break

#9
else:
    result = max(result, card_num[-1] + 100)
