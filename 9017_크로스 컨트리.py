...
for t in range(T):
    ...
    team_cnt = set(score)   #참가한 팀 확인
    remove_team = []    #6명 미만 참가 팀 분리
    for c in team_cnt:  #참가한 모든 팀 돌며
        if score.count(c) < 6:  #참가 인원이 6명 미만이면
            remove_team.append(c)   #분리

    score = [s for s in score if s not in remove_team]  #6명 이상 참가 팀들의 점수만 저장
    team_cnt = set(score)   #6명 이상 참가 팀 확인

    team_score = {t : [] for t in team_cnt} #각 팀별로 점수 분리할 딕셔너리

    for i in range(len(score)): #점수들 돌면서
        team = score[i]
        team_score[team].append(i+1)    #각 팀별로 점수 분리

    choose = [] #우승 팀 고를 리스트
    for team in team_score:
        choose.append((team, sum(team_score[team][:4]), team_score[team][4]))   #(팀 번호, 4명까지 점수 합, 5번째 선수 점수) 형태로 저장

    choose.sort(key = lambda x : (x[1], x[2]))  #4명까지 점수 합 -> 다섯번째 선수 점수 순으로 정렬
    print(choose[0][0]) #1등 팀 번호 출력
